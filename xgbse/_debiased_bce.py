...
class XGBSETimeVariantBCE(XGBSEBaseEstimator):
    """"""
    ...
    def fit(
        ...
        Args:
            ...
            time_bins (np.array): Specified time windows to use when making survival predictions, if not specified uses 12 equidistant time bins

            # TODO: Add new args to docs
        ...
        assert var_features.shape[1] == len(time_bins)
        ...
        # Using xgboost importance as feature importance
        self.feature_importances_ = self.bst.get_score()
        ...
        # fitting LR for several targets
        self.lr_estimators_ = self._fit_all_lr(
            leaves_encoded, self.targets, var_features
        )
    ...
    def _fit_one_lr(self, leaves_encoded, target, var_features):
        """
        Fits a single logistic regression to predict survival probability
        at a certain time bin as target. Encoded leaves are used as features.
        ...
        Args:
            ...
            # TODO: add var_features to docstrings
        ...
        # TODO: add shapes of things on docstrings

        # masking
        # TODO: add explanation for this mask
        mask = target != -1
        ...
        if len(target[mask]) == 0:
            ...
            classifier = DummyLogisticRegression()
        classifier_features = leaves_encoded[mask, :]
        classifier_features = hstack(
            [classifier_features, var_features.reshape(-1, 1)[mask, :]]
        )

        classifier.fit(classifier_features, target[mask])
        return classifier

    def _fit_all_lr(self, leaves_encoded, targets, var_features):
        """
        Fits multiple Logistic Regressions to predict survival probability
        for a list of time bins as target. Encoded leaves are used as features.
        ...
        with Parallel(n_jobs=self.n_jobs) as parallel:
            lr_estimators = parallel(
                delayed(self._fit_one_lr)(
                    leaves_encoded, targets[:, i], var_features[:, i]
                )
                for i in range(targets.shape[1])
            )
